...
void WirelessChannel::initialize(int stage)
{
    ...
    //Feito por Luan
    sumCurrentSignalReceived = 0;
    contSignal = 0;
    declareOutput("avgSignal");
}
...
*/
    ...
    case WC_SIGNAL_START:{
        ...
        for (it1 = pathLoss[cellTx].begin(); it1 != pathLoss[cellTx].end(); it1++) {
            ...
            // Feito por Luan
            //float currentSignalReceived = -95;
            //float currentSignalReceived = signalMsg->getPower_dBm() + (*it1)->avgPathLoss;
            ...
            //Feito por Luan
            if(sumCurrentSignalReceived > currentSignalReceived){
                sumCurrentSignalReceived = currentSignalReceived;
                contSignal++;
            }
...
{
    //Feito por Luan
    collectOutput("avgSignal", "avgSignal", sumCurrentSignalReceived);
...
float WirelessChannel::calcpathlossWUSN(double distance)
{
    if(distance <= 5){
        return 65;
    }else if(distance <= 10){
