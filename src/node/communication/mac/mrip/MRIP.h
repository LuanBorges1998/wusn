...
class MRIP: public VirtualMac
{
    ...
    void sendBeacon(bool, int);
    bool isChannelClear();
    void setState(MRIPState newState, int event);
	...
	protected:
		...
		bool isAck(MacPacket *);
};

class MripHeaderBase{// extends FieldsChunk{
	int srcAddr;
	int destAddr;
	MRIPType type;
};

class MripBeaconFrame: MripHeaderBase { };

class MripDataFrame: MripHeaderBase{
