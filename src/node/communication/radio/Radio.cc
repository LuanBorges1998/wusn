...
void Radio::initialize()
{
	...
	collectOutput("sensitivity", "sensitivity", RXmode->sensitivity);
	declareOutput("power");
			...
			if (endingSignal->bitErrors != ALL_ERRORS) {
				if (endingSignal->bitErrors <= maxErrorsAllowed(endingSignal->encoding)) {
					...
					if (endingSignal->maxInterference == RXmode->noiseFloor) {
						...
						//Aqui coloco o código a ser executado quando existe interferencia
						//cPacket *pkt = new cPacket();
						MacPacket *macFrame = new MacPacket("INTERFERENCE", MAC_LAYER_PACKET);
						sendDelayed(macFrame, PROCESSING_DELAY, "toMacModule");
					}
				}
			}
			//Personalizado Luan
